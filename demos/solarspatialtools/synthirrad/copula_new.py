...
def _copularnd_gaussian(n, cov_matrix, random_state=None):
    ...
    the standard normal CDF to transform them into uniform samples.

    Uses an explicit Cholesky factorization instead of
    rng.multivariate_normal, which defaults to a much slower SVD.
    ...
    L = _cholesky_factor(cov_matrix)

    # z = eps @ L.T gives rows distributed as N(0, C)
    eps = rng.standard_normal((n, cov_matrix.shape[0]))
    z = eps @ L.T
...
def _cholesky_factor(cov_matrix, max_tries=4):
    """
    Cholesky factorization with a small jitter fallback, in case the
    covariance matrix is numerically not positive definite (which can
    happen when two spacetime points nearly coincide).
    """
    jitter = 0.0
    for i in range(max_tries):
        try:
            ...
            jitter = 1e-10 if jitter == 0 else jitter * 100
    raise np.linalg.LinAlgError(
        "Covariance matrix is not positive definite even with jitter. "
        "Check for duplicate site positions or duplicate time stamps."
    )
